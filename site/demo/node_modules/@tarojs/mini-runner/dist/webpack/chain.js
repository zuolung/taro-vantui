...
exports.getResolveUrlLoader = fp_1.pipe(exports.mergeOption, lodash_1.partial(getLoader, 'resolve-url-loader'));
...
const getTerserPlugin = ([enableSourceMap, terserOptions]) => {
    return new TerserPlugin({
        ...
        terserOptions: helper_1.recursiveMerge({}, terserOptions_1.default, terserOptions)
    });
};
...
const getMiniSplitChunksPlugin = (args) => {
    return lodash_1.partial(getPlugin, MiniSplitChunksPlugin_1.default)([args]);
};
exports.getMiniSplitChunksPlugin = getMiniSplitChunksPlugin;
const getBuildNativePlugin = args => {
    return lodash_1.partial(getPlugin, BuildNativePlugin_1.default)([args]);
};
...
enableSourceMap, compile, cssLoaderOption, lessLoaderOption, sassLoaderOption, stylusLoaderOption, fontUrlLoaderOption, imageUrlLoaderOption, mediaUrlLoaderOption, postcss, fileType }) => {
    ...
    const sassLoader = exports.getSassLoader([baseSassOptions, {
            sassOptions: {
                indentedSyntax: true
            }
        }, sassLoaderOption]);
    const scssLoader = exports.getSassLoader([baseSassOptions, sassLoaderOption]);
    const resolveUrlLoader = exports.getResolveUrlLoader([{}]);
    ...
    const urlOptions = helper_1.recursiveMerge({}, defaultUrlOption, postcssOption.url);
...
const getEntry = ({ sourceDir, entry, isBuildPlugin }) => {
    ...
    Object.keys(pluginConfig).forEach(key => {
        if (key === 'main') {
            ...
            entryObj[pluginMainEntry] = [helper_1.resolveMainFilePath(filePath.replace(path.extname(filePath), ''))];
        }
            ...
            Object.keys(pluginConfig[key]).forEach(subKey => {
                ...
                entryObj[`plugin/${pluginConfig[key][subKey]}`] = [helper_1.resolveMainFilePath(filePath.replace(path.extname(filePath), ''))];
            });
        }
    });
    return {
